    ...
    <?php if ( $form_event_id ) : ?>
        ...
        <form id="spa-service-form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" enctype="multipart/form-data">
            ...
            <table class="form-table">
                ...
                <tr><th><label for="liturgy">Order of service</label></th><td><select name="liturgy" id="liturgy"><option value="">Select an LSB liturgy</option><?php foreach ( spa_services_get_liturgy_choices() as $liturgy ) : ?><option value="<?php echo esc_attr($liturgy); ?>" <?php selected($edit_service ? $edit_service->liturgy : '', $liturgy); ?>><?php echo esc_html($liturgy); ?></option><?php endforeach; ?></select><p class="description">The selected liturgy will appear above the hymns on the public service page.</p></td></tr>
                <tr><th><label for="sermon_text">Sermon full text</label></th><td><?php wp_editor($edit_service ? $edit_service->sermon_text : '', 'sermon_text', array('textarea_name' => 'sermon_text', 'textarea_rows' => 12, 'media_buttons' => false, 'teeny' => false, 'quicktags' => true)); ?></td></tr>
                ...
                <tr><th><label for="hymns">Hymns</label></th><td><textarea name="hymns" id="hymns" rows="5" class="large-text"><?php if ( ! empty($hymns) ) { foreach ( $hymns as $hymn ) { echo esc_textarea($hymn->reference . ($hymn->title || $hymn->author || $hymn->tune ? ' | ' . $hymn->title . ' | ' . $hymn->author . ' | ' . $hymn->tune : '') . "\n"); } } ?></textarea><p class="description">One reference per line, such as <code>LSB 617</code> or <code>CH4 221</code>. Optional fallback metadata: <code>Reference | Title | Author | Tune</code>.</p></td></tr>
